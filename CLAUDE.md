...
```
Requires a `.env` file (loaded via `python-dotenv`) with at least: `DATABASE_URL`, `ANTHROPIC_API_KEY`, `ANTHROPIC_ADMIN_KEY`. Optionally set `ANTHROPIC_WORKSPACE_ID` (e.g. `wrkspc_...`) to scope all cost/usage/pricing data (dashboard spend, per-model rates) to one Anthropic workspace instead of the whole org — recommended when the admin key's org has other workspaces/keys unrelated to this chatbot.
